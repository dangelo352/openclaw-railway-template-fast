...
### Slow builds / slow first provision

Cold provisioning speed is dominated by building OpenClaw from source. This template now includes:
- BuildKit cache mounts for `apt`, `pnpm`, and `npm` in `Dockerfile`
- `.dockerignore` to reduce Docker build context size
- `npm ci --omit=dev` for reproducible and cache-friendly wrapper installs

For faster repeated deployments on Railway:
- Enable **Skipped builds** in Railway service build settings (deploy cached image when source is unchanged)
- Keep `OPENCLAW_GIT_REF` pinned to a stable release unless you need latest `main`

For faster first deploys (new project / cold cache), use the prebuilt-image Dockerfile in this repo:
- Set `RAILWAY_DOCKERFILE_PATH=Dockerfile.prebuilt`
- This layers the wrapper on top of `ghcr.io/openclaw/openclaw:latest` and skips source compilation
- Keep `Dockerfile` (source build) as fallback if you need to pin or patch OpenClaw source directly
